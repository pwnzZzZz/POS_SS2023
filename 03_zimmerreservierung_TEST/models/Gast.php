...
class Gast implements DatabaseObject{
    ...
    public function create(){
        ...
        $stmt = $db->prepare($sql);
        $stmt->execute(array($this->name, $this->email, $this->adresse));

        $roomCreated = $stmt->fetchObject('Gast');
    ...
    public function update(){
        ...
        $sql = "UPDATE gast SET name = ?, email = ?, adresse = ? WHERE id = ?";
        $stmt = $db->prepare($sql);
        $stmt->execute(array($this->name, $this->email, $this->adresse, $this->id));

        $updatedGuest = $stmt->fetchObject('Gast');
        Database::disconnect();
